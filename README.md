...
<head>
    ...
    <style>
        ...
        @keyframes purr { 0% { transform: scale(1); } 50% { transform: scale(1.03) rotate(1deg); } 100% { transform: scale(1); } }
...
<script>
    ...
    // ОБНОВЛЕННЫЙ ГОЛОС СТИЧА
    function sayStitch(text, mood = 'normal') {
        const msg = new SpeechSynthesisUtterance(text);
        msg.lang = 'ru-RU';
        
        switch(mood) {
            case 'happy':
                msg.pitch = 2.0; // Очень высокий
                msg.rate = 1.3;  // Быстрый
                break;
            case 'ohana':
                msg.pitch = 0.5; // Низкий и хриплый
                msg.rate = 0.7;  // Медленный
                break;
            case 'hungry':
                msg.pitch = 1.2;
                msg.rate = 1.0;
    ...
    function update(action) {
        ...
        switch(action) {
            case 'feed':
                ...
                sayStitch("Ммм, кокос! Ням-ням!", "happy"); 
            ...
            case 'play':
                ...
                stats.energy -= 25;
                sayStitch("Иии-ха! Абакаба!", "happy");
                msg.innerText = "Серфинг! 🏄‍♂️";
                break;
            case 'hug':
                stats.mood = Math.min(100, stats.mood + 30);
                sayStitch("Охана значит семья", "ohana");
                msg.innerText = "Охана! 🫂";
                break;
            case 'sleep':
                ...
                if (isSleeping) sayStitch("Хррр-пссс", "ohana");
                else sayStitch("Алоха! Стич проснулся!", "happy");
                msg.innerText = isSleeping ? "Стич спит... 💤" : "Алоха! ✨";
                break;
        }
        refreshUI();
    }
    ...
    setInterval(() => {
        if (!isSleeping) {
            ...
            if (stats.hunger < 25 && stats.hunger > 0) {
                sayStitch("Стич хочет есть! Дай сэндвич!", "hungry");
                document.getElementById('status').innerText = "Голодный Стич! 😭";
            }
        } else {
